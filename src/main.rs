...
fn main() {
    ...
    println!("Starting the {} client(s), each client sending {} messages, with initial seed: {}", total_clients, total_messages_per_client, seed);
    ...
    let client_handler = thread::spawn(move || {
        match rx.recv_timeout(TIMEOUT) {
            Ok(Ok(_)) => Client.start_client(seed, total_clients, total_messages_per_client, full_server_address),
            Ok(Err(e)) => {
...
fn get_args() -> (u16, u64, usize, usize) {
    ...
    (
        ...
        args.next()
            .map(|a| a.parse().expect("invalid usize for total messages per client"))
